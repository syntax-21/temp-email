...
// Define default domain for emails
const EMAIL_DOMAIN = process.env.DOMAIN ? new URL(process.env.DOMAIN).hostname : 'tempmail.com'; // Fallback if not configured
...
// Handle /start command (the only command needed to open the menu)
...
// Handle text messages for state (e.g., custom email)
bot.on('message:text', async (ctx) => {
  ...
  if (state === 'awaiting_custom_email') {
    ...
    await kv.del(`bot_state:${chatId}`); // Clear state
  ...
  // If no state, just show the main menu
...
bot.callbackQuery('inbox', async (ctx) => {
  ...
  // Limit to 5 emails to avoid telegram message length limit
  const maxEmails = Math.min(emails.length, 5);
  for (let i = 0; i < maxEmails; i++) {
    const e = emails[i] as any; // The structure depends on postal-mime output stored
