...
import { Mail, RefreshCcw, Copy, Inbox, Sparkles } from 'lucide-react';
...
export default function TempMail() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#050505] text-gray-200 font-sans overflow-hidden selection:bg-indigo-500/30">
      
      {/* Background Ambient Glow */}
      <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full bg-indigo-600/20 blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px] rounded-full bg-purple-600/10 blur-[150px] pointer-events-none"></div>

      {/* LEFT SIDEBAR (Glassmorphism) */}
      <aside className="w-72 bg-white/[0.02] backdrop-blur-3xl flex flex-col p-7 z-20 shrink-0 border-r border-white/[0.05] relative">
        <div className="flex items-center gap-3 text-white mb-10 mt-2">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full bg-black/40 border border-white/[0.08] rounded-xl px-4 py-3.5 text-gray-200 placeholder-gray-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all text-sm"
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select 
                  disabled
                  className="w-full bg-black/40 border border-white/[0.08] rounded-xl px-4 py-3.5 text-gray-400 appearance-none opacity-80 cursor-not-allowed text-sm"
                >
        ...
        <div className="mt-auto pt-8">
          <div 
            ...
            className="group relative bg-black/30 border border-white/[0.05] hover:border-indigo-500/30 rounded-2xl p-4 cursor-pointer transition-all overflow-hidden"
          >
            ...
            <span className="font-mono text-gray-200 text-sm break-all leading-tight relative z-10">
              {emailAddress || '...'}
            </span>
            {copied && (
      ...
      <section className="w-[340px] lg:w-[380px] bg-[#0a0a0c]/80 backdrop-blur-xl border-r border-white/[0.05] flex flex-col shrink-0 z-10">
      ...
      <main className="flex-1 bg-transparent flex flex-col relative overflow-hidden z-10">
        {selectedEmail ? (
          <div className="flex-1 flex flex-col h-full overflow-hidden m-4 lg:m-6 bg-white/[0.02] border border-white/[0.05] rounded-3xl shadow-2xl backdrop-blur-sm">
            {/* Email Header Info */}
            <div className="p-8 border-b border-white/[0.05] bg-gradient-to-b from-white/[0.03] to-transparent shrink-0">
              <h2 className="text-2xl font-bold text-gray-100 mb-6 leading-snug">{selectedEmail.subject}</h2>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-600/20 text-indigo-400 flex items-center justify-center font-bold text-xl shrink-0 border border-indigo-500/20 shadow-inner">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 flex items-center gap-2">
                    <span>{new Date(selectedEmail.receivedAt).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
            ...
            {/* Email Body Viewer */}
            <div className="flex-1 overflow-auto p-0 bg-transparent rounded-b-3xl">
              {selectedEmail.html ? (
                <div className="w-full h-full bg-[#f8fafc] transition-all relative">
                  {/* Subtle border to frame the white content nicely inside the dark UI */}
                ...
                <div className="p-8 text-sm font-mono text-gray-300 whitespace-pre-wrap h-full overflow-auto leading-relaxed bg-[#050505]">
